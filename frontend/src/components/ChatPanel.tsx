    ...
    <>
      ...
      {showChat && streamStatus === 'idle' && tokenStats && (
        <div className="panel-status-bar done">
          ...
          <div className="psb-right">
            <span className="psb-tokens" title={`输入 ${formatTokens(tokenStats.prompt)} · 输出 ${formatTokens(tokenStats.completion)}`}>
              {formatTokens(tokenStats.total)} tokens
              <span className="psb-token-breakdown">
                <span className="psb-tb-in">←{formatTokens(tokenStats.prompt)}</span>
